import { SectionEyebrow, SectionTitle, SectionSub } from "@/components/section/SectionHeader";
...
function DeliverablesCard({ items }: { items: { title: string; sub: string }[] }) {
  return (
    <div className="bg-(--bg-card) border border-(--border) rounded-xl p-8">
      <h4 className="font-(--font-sans) text-[.75rem] uppercase tracking-widest text-white/30 mb-5">
        What you get
      </h4>
      {items.map((d) => (
        <div
          key={d.title}
          className="flex items-start gap-3.5 py-3.5 border-b border-(--border) last:border-0"
        >
          <div className="w-[18px] h-[18px] rounded bg-[rgba(91,141,238,.15)] text-(--acc2) flex items-center justify-center text-[.6rem] shrink-0 mt-0.5">
            ✓
          </div>
          <div>
            <h5 className="font-(--font-sans) text-[.88rem] text-(--text-hi) mb-1">
              {d.title}
            </h5>
            <p className="text-[.8rem] text-(--text-dim) leading-[1.55]">
              {d.sub}
            </p>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-(--border) border-[1.5px] border-(--border) rounded-xl overflow-hidden mt-12">
          {SERVICES.map((s) => (
            <div
              key={s.num}
              className="rv rv-d2 bg-[rgba(10,18,69,.95)] p-10 pr-8 relative transition-colors hover:bg-[rgba(90,107,187,1)] group"
            >
              <div className="absolute top-7 right-7 text-white/10 group-hover:text-(--acc2) group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all">
                ↗
              </div>
              <div className="font-(--font-serif) text-4xl italic text-white/6 mb-5">
                {s.num}
              </div>
              <div className="text-[.68rem] uppercase tracking-widest text-(--acc2) bg-[rgba(91,141,238,.12)] py-1 px-2.5 rounded inline-block mb-3.5 font-medium">
                {s.tag}
              </div>
              <h3 className="text-xl font-semibold mb-2.5 text-white">
                {s.title}
              </h3>
              <p className="text-[.86rem] text-(--text-dim) leading-7">
                {s.desc}
              </p>
            </div>
          ))}
        ...
        <div className="rv grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-16 items-start py-16 border-t border-(--border)">
          <div>
            <div className="text-[.68rem] uppercase tracking-widest text-(--acc2) bg-[rgba(91,141,238,.12)] py-1 px-2.5 rounded inline-block mb-3 font-medium">
            ...
            <p className="text-[.95rem] text-(--text-mid) leading-[1.85] mb-4">
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {["Next.js", "React", "TypeScript", "MongoDB", "Node.js", "Prisma", "Tailwind CSS", "Vercel / AWS"].map(
                (t) => (
                  <span
                    key={t}
                    className="text-[.72rem] py-1 px-2.5 rounded-full bg-[rgba(91,141,238,.08)] border border-[rgba(91,141,238,.2)] text-(--acc3)"
                  >
                    {t}
                  </span>
                )
        ...
        <div className="rv bg-[rgba(91,141,238,.06)] border border-[rgba(91,141,238,.18)] rounded-xl p-10 mt-14 flex flex-col sm:flex-row justify-between items-center gap-8 flex-wrap">
          <p className="font-(--font-serif) text-[1.1rem] max-w-[440px] leading-[1.55] text-(--text-mid) [&_em]:text-(--acc2) [&_em]:italic">
