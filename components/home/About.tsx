...
export default function About() {
  return (
    <section
      id="about"
      className="relative border-t border-[var(--border)] py-28 px-[5vw] scroll-mt-16 bg-blue-100">
      <div className="mx-auto max-w-[1100px] grid grid-cols-1 sm:grid-cols-[1.4fr_1fr] gap-10 lg:gap-16">
        ...
        <div className="space-y-6">
          ...
          <p className="flex items-center gap-2.5 mb-4 text-[.7rem] font-medium uppercase tracking-[.14em] text-blue-900/60">
            <span className="w-6 h-px bg-[var(--acc2)] opacity-60 shrink-0" aria-hidden />
            WHO WE ARE
            <span className="w-6 h-px bg-[var(--acc2)] opacity-60 shrink-0" aria-hidden />
          </p>

          <h2 className="font-[var(--font-serif)] text-[clamp(2rem,4vw,3.25rem)] font-semibold tracking-tight text-[var(--bg-deep)] leading-[1.1] [&_em]:italic [&_em]:font-light [&_em]:text-[var(--acc2)]">
            Your home-grown team
            <br />
            <em>with a global outlook.</em>
          </h2>

          <div className="space-y-5 text-[.98rem] leading-[1.85] [&_strong]:text-[var(--bg-deep)] [&_em]:text-[var(--acc2)] text-slate-700">
            <p>
        ...
        <div className="flex flex-col gap-4">
          {pillars.map(({ id, symbol, title, description }) => (
            <div
              ...
              <h3 className="font-[var(--font-sans)] text-base font-semibold text-[var(--bg-deep)] mb-1.5">
                {title}
              </h3>
