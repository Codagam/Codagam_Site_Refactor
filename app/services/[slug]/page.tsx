  ...
  return (
    <div className="min-h-screen w-full bg-(--bg-deep)">
      ...
      <main className="relative z-10 w-full min-w-0 pt-4 sm:pt-5 md:pt-6 lg:pt-8 overflow-x-hidden">
                ...
                {service.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {service.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-(--border) bg-[rgba(255,255,255,.04)] px-3 py-1.5 text-[.74rem] text-(--text-mid)">
                        {tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-5">
              {SECURITY_ITEMS.map((item, i) => (
                <div
                  ...
                  <p className="text-[.84rem] text-(--text-dim) group-hover:text-slate-600 leading-[1.7] transition-colors">
                    {item.text}
                  </p>
                </div>
              ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px overflow-hidden rounded-xl border border-(--border) bg-(--border)">
              {PROCESS_STEPS.map((step, i) => (
                <div
                  ...
                  <p className="text-[.83rem] text-(--text-dim) group-hover:text-slate-600 leading-[1.7] transition-colors">
                    {step.text}
                  </p>
                </div>
              ))}
        ...
        <div className="relative z-20 border-t border-(--border) bg-(--bg-deep)">
          <Footer />
        </div>
      </main>
    </div>
